<script lang="ts">
   ...
   *
   * Neumorphic: see TactileCheckbox's doc comment — same raised/inset/
   * gradient-glow treatment, shared via neumorphic.css.
   */
  ...
  let { pinned, disabled = false, label, accent = 'notes', onToggle }: Props = $props();
...
<button
  ...
  class="relative flex h-6 w-6 shrink-0 items-center justify-center rounded-xl
         ...
         {pinned ? `accent-${accent} accent-${accent}-glow` : isPressed ? 'neu-pressed' : 'neu-raised-sm'}
...
>
  {#if pinned}
    <svg viewBox="0 0 24 24" class="h-3.5 w-3.5 text-white" fill="none" aria-hidden="true">
      <path
